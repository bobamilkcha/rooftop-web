...
const IntroPowerDirect = () => {
    ...
    return (
        <section className="relative py-15 lg:py-30 bg-[#141624] text-white px-8 md:px-12 lg:px-24 xl:px-32">

            <div className="space-y-1">
                <div className="text-4xl font-semibold text-[#FCD913]">
            ...
            <div className="my-14 w-5/6 sm:w-5/12 lg:w-full mx-auto">
                <div className="grid gap-2 lg:gap-6 xl:gap-12 space-y-8 lg:space-y-0 lg:grid-cols-4">
                    ...
                    <div key={index} className="bg-[#222634] rounded-xl px-6 py-6 shadow-sm flex flex-col" >
                        <div className="flex flex-col items-center text-center">
                        <div>{item.icon}</div>
                        <div className="text-xl font-semibold mt-6 lg:mt-8">{item.title}</div>
                        </div>
                        <p className="text-lg font-light text-[#D3D4D9] mt-6 text-center">{item.description}</p>
                    </div>
                    ))}
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-6 gap-6 items-stretch">
                <div className="col-span-1 lg:col-span-3">
                    <button className="h-full w-full bg-[#FCD913] text-black text-xl font-semibold px-4 md:px-6 py-5 rounded-2xl flex justify-between transition">
                    Power Direct <BsArrowRight className="text-xl" />
                    </button>
                </div>
                <div className="col-span-1 lg:col-span-3">
                    <div className="w-full bg-[#222634] text-white px-4 md:px-6 py-5 rounded-2xl flex flex-col justify-between">
                        <div className="font-semibold flex items-center justify-between text-xl">
                            All financing plans <BsArrowRight className="text-[#FCD913] text-xl" />
                        </div>
                        <div className="font-mono text-base text-[#D3D4D9] mt-4">Upfront payment, installments, and solar maintenance solutions.</div>
                    </div>
                </div>
            </div>
        </section>
